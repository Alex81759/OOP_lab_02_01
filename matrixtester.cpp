...
// деление на 0
// напиши минимальное и достаточное количество тестов для покрытия всей программы
...
void MatrixTester::runSingleTest(OutputPrinter& output, const std::string& testName, void (MatrixTester::*testFunction)() const, int& passedCount, int& failedCount) const {
    try {
        (this->*testFunction)();
        ++passedCount;
